...
def recurring_routine_intervals(profile: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for item in routine_items:
        ...
        if not days:
            ...
            continue
        explicit_shift = item.get("shift_minutes") is not None
        shift_minutes = max(int(item.get("shift_minutes") or 0), 0)
        available_by_day = {int(window["day_index"]): window for window in parse_available_windows(profile)}
        for day in days:
            ...
            intervals.append({
                ...
                "availability_window": (
                    {"start": max(0.0, float(start) - shift_minutes / 60), "end": min(24.0, float(end) + shift_minutes / 60)}
                    if explicit_shift
                    else {
                        "start": float(available_by_day.get(day, {"start": start})["start"]),
                        "end": float(available_by_day.get(day, {"end": end})["end"]),
                    }
                ),
                "movable": True,
                ...
                "confidence": item.get("confidence") or (defaults or {}).get("confidence") or "high",
                    "evidence": exception.get("reason") or item.get("evidence") or [value for value in [(defaults or {}).get("label"), (f"May shift by up to {shift_minutes} minutes" if explicit_shift else "A preferred routine time; movable within the available window when needed")] if value],
            })
    return intervals, uncertain
