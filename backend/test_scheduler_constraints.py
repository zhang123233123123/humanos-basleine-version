...
class SchedulerConstraintTests(unittest.TestCase):
    ...
    def test_lunch_routine_uses_visible_low_confidence_default_and_is_not_hard(self) -> None:
        ...
        by_day = {item["day_index"]: item["availability_window"] for item in routines}
        self.assertEqual({"start": 8.0, "end": 21.0}, by_day[1])
        self.assertEqual({"start": 8.0, "end": 18.0}, by_day[2])
        self.assertEqual({"start": 8.0, "end": 18.0}, by_day[3])
        self.assertEqual({"start": 12.0, "end": 13.0}, by_day[0])
