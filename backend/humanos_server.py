...
class Store:
    ...
    def init_db(self) -> None:
        with self.connect() as conn:
            ...
            execution_columns = {
                row["name"] for row in conn.execute("PRAGMA table_info(execution_sessions)").fetchall()
            }
            if "timeline_json" not in execution_columns:
                conn.execute("ALTER TABLE execution_sessions ADD COLUMN timeline_json TEXT NOT NULL DEFAULT '[]'")
            conn.execute(
            ...
            self._repair_deadline_misclassified_tasks(conn)

    def _repair_deadline_misclassified_tasks(self, conn: sqlite3.Connection) -> None:
        """Repair legacy Weekly Setup tasks whose deadline clock became a start time.

        Older builds inferred ``fixed_event`` from any clock, including a deadline
        such as Friday 18:00. Genuine meetings/classes remain fixed because their
        wording still passes ``infer_schedule_task_type`` as a fixed event.
        """
        affected_users: set[str] = set()
        rows = conn.execute(
            "SELECT id,user_id,title,due,status,context,context_window_json FROM tasks"
        ).fetchall()
        for row in rows:
            context_window = dict(from_json(row["context_window_json"], {}) or {})
            if str(context_window.get("taskType") or "") != "fixed_event":
                continue
            inferred = self.infer_schedule_task_type({
                "title": row["title"],
                "due": row["due"],
                "context": row["context"],
            })
            if inferred != "flexible_task":
                continue
            context_window["taskType"] = "flexible_task"
            status = "queued" if row["status"] in {"scheduled", "partially_scheduled"} else row["status"]
            conn.execute(
                "UPDATE tasks SET context_window_json=?,slot_json=NULL,status=?,updated_at=? WHERE id=? AND user_id=?",
                (as_json(context_window), status, now_ms(), row["id"], row["user_id"]),
            )
            affected_users.add(str(row["user_id"]))
        for user_id in affected_users:
            conn.execute(
                "UPDATE plans SET plan_status='superseded',updated_at=? WHERE user_id=? AND plan_status='proposed'",
                (now_ms(), user_id),
            )
    ...
    def infer_schedule_task_type(self, payload: dict) -> str:
        ...
        # A deadline clock is a latest-finish constraint, never evidence that work
        # starts at that clock time. Only the task wording itself may express a
        # fixed start (for example, "research meeting at 15:00").
        task_text = f"{title} {context}"
        task_has_clock = re.search(
            r"(?:\d{1,2}|十二|十一|十|[一二两三四五六七八九])\s*(?:点|时)(?!间)|"
            r"\d{1,2}[:：]\d{2}|\d{1,2}(?::\d{2})?\s*(?:am|pm)",
            task_text,
            re.I,
        )
        due_has_clock = re.search(
            r"(?:\d{1,2}|十二|十一|十|[一二两三四五六七八九])\s*(?:点|时)(?!间)|"
            r"\d{1,2}[:：]\d{2}|\d{1,2}(?::\d{2})?\s*(?:am|pm)",
            due,
            re.I,
        ...
        deadline_words = re.search(r"(截止|ddl|deadline|之前|以前|前完成|due|\bby\b|before)", task_text, re.I)
        if explicit in {"flexible_task", "fixed_event", "recovery_task"}:
            ...
            return explicit
        if fixed_words and (task_has_clock or due_has_clock):
            return "fixed_event"
        return "flexible_task"
    ...
    def chat_turn(self, user_id: str, payload: dict) -> dict:
        ...
        if context_update:
            ...
            response["reply"] = (
                ...
                + ("This is a preferred routine time. I will preserve it when possible and may move it within your available time when needed." if item.get("type") == "recurring_routine" else "This is fixed time. I will not search for another position; I will keep it there, check conflicts, and generate one revised draft plan around it.")
            )
        ...
        with self.connect() as conn:
            ...
            for draft in submitted:
                ...
                if task_id:
                    ...
                    schedule_type = self.infer_schedule_task_type({
                        **draft,
                        "title": draft.get("title") or before.get("title"),
                        "due": draft.get("due", before.get("due")),
                        "context": draft.get("context", before.get("context")),
                    })
                    if schedule_type != schedule_task_kind(before):
                        scope.add("task_type")
                        reschedule_fields.add("task_type")
                    context_window["taskType"] = schedule_type
    ...
    def confirm_plan(self, user_id: str, payload: dict) -> dict:
        ...
        with self.connect() as conn:
            ...
            for block in decorated:
                ...
                block_id = str(block.get("block_id") or new_id("block"))
                # A paused/running execution is an actual historical object, not a
                # disposable copy of a Plan block. A local Plan revision may move
                # its expected continuation, but must retain the same Session ID,
                # active-time timeline and context dump.
                live_execution = conn.execute(
                    "SELECT id,status FROM execution_sessions WHERE user_id=? AND block_id=? AND status IN ('running','paused') ORDER BY updated_at DESC LIMIT 1",
                    (user_id, block_id),
                ).fetchone()
                if live_execution:
                    conn.execute(
                        "UPDATE execution_sessions SET week_id=?,plan_revision=?,planned_start_at=?,planned_end_at=?,planned_work_minutes=MAX(planned_work_minutes,?),updated_at=? WHERE id=? AND user_id=?",
                        (week_id, revision, str(block.get("start_at") or ""), str(block.get("end_at") or ""), planned_minutes, timestamp, live_execution["id"], user_id),
                    )
                else:
                    ...
                    conn.execute(
                        ...
                        (execution_id, user_id, str(block.get("task_id") or ""), block_id, week_id, revision, str(block.get("start_at") or ""), str(block.get("end_at") or ""), planned_minutes, "ready", timestamp, timestamp),
                    )
    ...
    def execution_session_row(self, row: sqlite3.Row) -> dict:
        ...
        data["timeline"] = from_json(data.pop("timeline_json", "[]"), [])
        return data
    ...
    def current_execution(self, user_id: str) -> dict:
        ...
        now = clock_now(ZoneInfo((self.ensure_profile(user_id).get("timezone") or "Asia/Shanghai")))
        future_ready, expired_ready = [], []
        for item in ready:
            try:
                planned_end = datetime.fromisoformat(str(item.get("planned_end_at") or ""))
                (expired_ready if planned_end <= now else future_ready).append(item)
            except ValueError:
                future_ready.append(item)
        # Old feedback never hides a real current/up-next session.
        selected = (running or paused or future_ready[:1] or ended or expired_ready[:1])
        if not selected:
        ...
        if mode == "up_next" and session.get("planned_start_at"):
            try:
                ...
                planned_end = datetime.fromisoformat(session["planned_end_at"]) if session.get("planned_end_at") else None
                now = clock_now(planned_start.tzinfo)
                if planned_end and now >= planned_end:
                    # A ready block that expired without Start is an ended,
                    # zero-active-minute session in the UI. Keep the stored
                    # status ready so "Start now" can still begin it.
                    mode = "session_ended"
    ...
    def start_execution_session(self, user_id: str, payload: dict) -> dict:
        ...
        with self.connect() as conn:
            ...
            event_type = "resume" if row["status"] == "paused" else "start"
            timeline_json = self._append_execution_timeline(row, event_type, actual_start, int(row["accumulated_active_minutes"] or 0))
            conn.execute(
                "UPDATE execution_sessions SET status='running',actual_start_at=COALESCE(actual_start_at,?),resumed_at=?,paused_at=NULL,request_id=COALESCE(request_id,?),timeline_json=?,updated_at=? WHERE id=? AND user_id=?",
                (actual_start, actual_start, request_id, timeline_json, timestamp, row["id"], user_id),
            )
    ...
    def pause_execution_session(self, user_id: str, payload: dict) -> dict:
        ...
        adjustment = payload.get("tracked_time_adjustment") if isinstance(payload.get("tracked_time_adjustment"), dict) else None
        request_id = str(payload.get("request_id") or "").strip() or None
        with self.connect() as conn:
            replay = self._execution_request_seen(conn, user_id, request_id)
            if replay:
        ...
        with self.connect() as conn:
            ...
            if adjustment is not None:
                ...
                if not reason:
                    raise ValueError("A reason is required when correcting tracked active time")
                active = max(int(adjustment.get("minutes") or 0), 0)
            timeline_json = self._append_execution_timeline(row, "end", ended_at, active)
            conn.execute("UPDATE execution_sessions SET status='ended',actual_end_at=?,resumed_at=NULL,accumulated_active_minutes=?,completion_outcome=NULL,timeline_json=?,updated_at=? WHERE id=? AND user_id=?", (ended_at, active, timeline_json, timestamp, session_id, user_id))
    ...
    def save_execution_feedback(self, user_id: str, payload: dict) -> dict:
        ...
        if execution_session_id:
            with self.connect() as conn:
                existing = conn.execute(
                    "SELECT * FROM execution_feedback WHERE user_id=? AND execution_session_id=? ORDER BY created_at DESC LIMIT 1",
                    (user_id, execution_session_id),
                ).fetchone()
            if existing:
                return {
                    "id": existing["id"], "user_id": existing["user_id"], "task_id": existing["task_id"],
                    "trigger": existing["trigger"],
                    "task_evaluation": from_json(existing["task_evaluation_json"], {}),
                    "state_evaluation": from_json(existing["state_evaluation_json"], {}),
                    "recommendation_evaluation": from_json(existing["recommendation_evaluation_json"], {}),
                    "execution_session_id": existing["execution_session_id"], "request_id": existing["request_id"],
                    "research_context_revision": existing["research_context_revision"], "created_at": existing["created_at"],
                    "task": self.get_task(task_id, user_id), "idempotent_replay": True,
                }
        session_row = None
        if execution_session_id:
            with self.connect() as conn:
                session_row = conn.execute(
                    "SELECT * FROM execution_sessions WHERE id=? AND user_id=? AND task_id=?",
                    (execution_session_id, user_id, task_id),
                ).fetchone()
            if not session_row:
                raise KeyError(execution_session_id)
        submitted_task_evaluation = dict(payload.get("task_evaluation") or {})
        completion = str(submitted_task_evaluation.get("completion") or "partial")
        tracked_active = int(session_row["accumulated_active_minutes"] or 0) if session_row else 0
        submitted_task_evaluation["actual_minutes"] = 0 if completion in {"not_started", "did_not_start"} else tracked_active
        feedback = {
            "id": new_id("feedback"),
            "user_id": user_id,
            "task_id": task_id,
            "trigger": payload.get("trigger", "task_completed"),
            "task_evaluation": submitted_task_evaluation,
        ...
        with self.connect() as conn:
            ...
            if feedback["execution_session_id"]:
                ...
                conn.execute(
                    "UPDATE execution_sessions SET status=?,completion_outcome=?,actual_end_at=COALESCE(actual_end_at,?),updated_at=? WHERE id=? AND user_id=?",
                    (session_status, outcome, clock_now(ZoneInfo(feedback_profile.get("timezone") or "Asia/Shanghai")).isoformat(), feedback["created_at"], feedback["execution_session_id"], user_id),
                )
        execution = task.get("execution") or {}
        ...
        # One source of truth: sum persisted active intervals from every
        # Execution Session for this task. Feedback never adds minutes again.
        with self.connect() as conn:
            total_active = conn.execute(
                "SELECT COALESCE(SUM(accumulated_active_minutes),0) AS total FROM execution_sessions WHERE user_id=? AND task_id=?",
                (user_id, task_id),
            ).fetchone()["total"]
        execution["accumulated_actual_minutes"] = int(total_active or 0)
        if completion in {"no_progress", "not_started", "did_not_start"}:
            execution["remaining_duration_minutes"] = int(execution.get("remaining_duration_minutes") or task.get("duration") or 0)
        elif task_eval.get("remaining_duration_minutes") is not None:
        ...
        if task_eval.get("completion") == "completed":
            ...
            patch["status"] = "queued"
        updated_task = self.patch_task(task_id, patch, user_id)
        self.add_memory(
            user_id=user_id,
            source_type="episodic_memory",
        ...
        return {**feedback, "task": updated_task}
    ...
    def validate_confirmed_schedule(self, user_id: str, payload: dict) -> dict:
        ...
        for raw in payload.get("plan_patch") or []:
            ...
            if not task:
                violations.append({"type": "unknown_task", "task_id": task_id, "block_id": block.get("block_id")})
                continue
            ...
            if schedule_task_kind(task) != "fixed_event":
                if not any(window["day_index"] == day and start >= window["start"] - 0.001 and end <= window["end"] + 0.001 for window in windows):
                    inside_minutes = sum(
                        max(0, int(round((min(end, float(window["end"])) - max(start, float(window["start"]))) * 60)))
                        for window in windows if window["day_index"] == day and min(end, float(window["end"])) > max(start, float(window["start"]))
                    )
                    outside_minutes = max(int(round((end - start) * 60)) - inside_minutes, 0)
                    violations.append({"type": "outside_available_window", "task_id": task_id, "block_id": block.get("block_id"), "day_index": day, "start": start, "end": end, "outside_minutes": outside_minutes})
                ...
                if conflict:
                    overlap_start = max(start, float(conflict.get("start", start)))
        ...
        explained = self._explain_schedule_violations(violations, task_map, blocks)
        return {"valid": not explained, "violations": explained, "checked_by": "python_hard_constraint_validator"}

    def _explain_schedule_violations(self, violations: list[dict], task_map: dict[str, dict], blocks: list[dict]) -> list[dict]:
        """Turn validator facts into stable, user-actionable conflict records."""
        days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
        block_map = {str(item.get("block_id")): item for item in blocks if item.get("block_id")}
        ...
        for raw in violations:
            item = dict(raw)
            kind = str(item.get("type") or "schedule_conflict")
            related_ids = [str(value) for value in (item.get("task_ids") or []) if value]
            item.setdefault("task_id", item.get("first_task_id") or item.get("before_task_id") or (related_ids[0] if related_ids else None))
            item.setdefault("conflicting_task_id", item.get("after_task_id") or (related_ids[1] if len(related_ids) > 1 else None))
            task = task_map.get(str(item.get("task_id") or "")) or {}
            other = task_map.get(str(item.get("conflicting_task_id") or "")) or {}
            if not task and item.get("block_ids"):
                first = block_map.get(str(item["block_ids"][0])) or {}
                task = task_map.get(str(first.get("task_id") or "")) or {}
                item.setdefault("task_id", first.get("task_id"))
            if not other and item.get("block_ids") and len(item["block_ids"]) > 1:
                second = block_map.get(str(item["block_ids"][1])) or {}
                other = task_map.get(str(second.get("task_id") or "")) or {}
                item.setdefault("conflicting_task_id", second.get("task_id"))
            day_index = item.get("day_index")
            day_label = days[int(day_index)] if day_index is not None and 0 <= int(day_index) < 7 else None
            start_label, end_label = clock(item.get("start")), clock(item.get("end"))
            minutes = max(0, int(round((float(item.get("end")) - float(item.get("start"))) * 60))) if item.get("start") is not None and item.get("end") is not None else None
            task_title = task.get("title") or str(item.get("task_id") or "Unknown task")
            conflicting_title = other.get("title") or item.get("conflicting_item_title") or item.get("constraint")
            when = " ".join(part for part in [day_label, f"{start_label}–{end_label}" if start_label and end_label else None] if part)
            if kind == "overlap":
                message = f"{task_title} overlaps {conflicting_title or 'another task'} for {minutes} minutes on {when}."
                solutions = [{"action": "move_task", "label": f"Move {task_title}"}, {"action": "move_conflicting_item", "label": f"Move {conflicting_title or 'the other task'}"}, {"action": "ask_humanos", "label": "Ask HumanOS to repair"}]
            elif kind == "hard_constraint_conflict":
                message = f"{task_title} conflicts with {conflicting_title or 'protected time'} for {minutes} minutes on {when}."
                solutions = [{"action": "move_task", "label": f"Move {task_title}"}, {"action": "add_available_time", "label": "Add available time"}, {"action": "ask_humanos", "label": "Ask HumanOS to repair"}]
            elif kind == "outside_available_window":
                minutes = int(item.get("outside_minutes") if item.get("outside_minutes") is not None else minutes or 0)
                message = f"{minutes} minutes of {task_title} fall outside the time HumanOS may use on {when}."
                solutions = [{"action": "move_task", "label": "Move inside available time"}, {"action": "add_available_time", "label": "Add available time"}]
            elif kind == "deadline":
                message = f"{task_title} ends after its deadline ({item.get('deadline') or 'deadline not resolved'})."
                solutions = [{"action": "move_task", "label": "Move before deadline"}, {"action": "move_deadline", "label": "Change deadline"}, {"action": "reduce_scope", "label": "Reduce scope"}]
            elif kind == "unexplained_unallocated_work":
                missing = max(int(item.get("remaining") or 0) - int(item.get("planned") or 0), 0)
                minutes = missing
                message = f"{missing} minutes of {task_title} are not scheduled before its deadline."
                solutions = [{"action": "add_available_time", "label": "Add available time"}, {"action": "move_deadline", "label": "Move deadline"}, {"action": "reduce_scope", "label": "Reduce scope"}]
            else:
                message = item.get("detail") or f"{task_title} has a {kind.replace('_', ' ')} that must be resolved."
                solutions = [{"action": "edit_task", "label": f"Edit {task_title}"}, {"action": "ask_humanos", "label": "Ask HumanOS"}]
            identity = json.dumps({"type": kind, "task": item.get("task_id"), "other": item.get("conflicting_task_id") or item.get("conflicting_item_id"), "day": day_index, "start": item.get("start"), "end": item.get("end")}, sort_keys=True)
            item.update({
                "violation_id": str(uuid.uuid5(uuid.NAMESPACE_URL, identity)), "task_title": task_title,
                "conflicting_task_title": conflicting_title, "date_label": day_label,
                "start_time": start_label, "end_time": end_label, "overlap_minutes": minutes,
                "message": message, "solutions": solutions,
            })
    ...
    def validate_llm_schedule_candidates(self, state: dict, decision: dict, llm_result: dict) -> list[dict]:
        ...
        for candidate_index, raw_candidate in enumerate(raw_candidates or []):
            ...
            violations = self._explain_schedule_violations(violations, task_map, blocks)
            validated.append({
                "id": candidate_id,
