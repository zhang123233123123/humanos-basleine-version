...
class HelpDecideStoreIntegrationTests(unittest.TestCase):
    ...
    @patch("backend.humanos_server.embedding_for_text", return_value=([0.0] * 64, "test"))
    def test_three_rejected_recommendations_form_candidate_without_profile_mutation(self, _embedding) -> None:
    ...
    @patch("backend.humanos_server.embedding_for_text", return_value=([0.0] * 64, "test"))
    def test_user_confirmation_promotes_candidate_without_plan_write(self, _embedding) -> None:
