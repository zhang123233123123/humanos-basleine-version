...
import { ArrowLeft, CalendarCheck, Loader2, RefreshCw, Sparkles } from 'lucide-react'
...
export default function WeeklyPlanPage() {
  ...
  const loadPlanningState = useCallback(async () => {
    setLoading(true)
    try {
      const [status, profileData, planData, proposedData] = await Promise.all([
        ...
        apiRequest<{ plan: PlanDecision | null }>('/api/plans/revisions/current'),
      ])
      ...
      if (proposedData.plan) {
        setDecision(proposedData.plan)
        setBlocks(proposedData.plan.plan_patch || [])
        setValidation(proposedData.plan.validation || null)
        setStage('review')
      } else if (planData.plan?.plan_status === 'confirmed') {
        setDecision(null)
        setBlocks([])
        setValidation(null)
        setStage('confirmed')
  ...
  const cancelRevision = async () => {
    if (!decision?.plan_id || !decision?.base_plan_id) return
    setSubmitting(true)
    try {
      await apiRequest('/api/plans/revisions/cancel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          plan_id: decision.plan_id,
          base_plan_id: decision.base_plan_id,
        }),
      })
      toast('Current calendar kept unchanged.')
      await loadPlanningState()
    } catch (error) {
      toast(error instanceof Error ? error.message : 'Failed to cancel the draft change')
    } finally {
      setSubmitting(false)
    }
  }
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between pt-3">
                  <Button
                    variant="outline"
                    onClick={decision.base_plan_id ? cancelRevision : () => setStage('setup')}
                    disabled={submitting}
                  >
                    {decision.base_plan_id ? 'Keep current plan' : t('planning.backToSetup')}
                  </Button>
                  <Button onClick={confirmPlan} disabled={submitting || validation?.valid === false || (rationaleRequired && !rationale.trim())}><CalendarCheck className="mr-2 h-4 w-4" />{decision.base_plan_id ? 'Apply changes' : 'Add plan to calendar'}</Button>
                </div>
              </CardContent>
            </Card>
            <div className="space-y-4">
              {validation && !validation.valid && <Card className="border-destructive/40"><CardHeader><CardTitle className="text-lg">HumanOS needs one decision</CardTitle><CardDescription>{t('planning.invalidPlan')}</CardDescription></CardHeader><CardContent className="space-y-2">{validation.violations?.map((item, index) => <p key={index} className="rounded-md bg-destructive/10 p-2 text-sm text-destructive">{item.message || item.type}</p>)}</CardContent></Card>}
