      ...
      <aside className="w-72 border-l border-border bg-background overflow-y-auto shrink-0">
        <div className="sticky top-0 z-20 border-b border-border bg-background/95 p-4 backdrop-blur">
          <div className="space-y-3">
            ...
            >
              {isConfirmingAll ? 'Adding tasks...' : 'Add tasks to weekly plan'}
            </Button>
          </div>
        </div>
  ...
  const handleUpdateEvent = async (event: EventDropArg | EventResizeDoneArg) => {
    ...
    const edit = {
      ...
      eventType: ('oldEvent' in event && event.event.start?.getTime() === oldEvent.start?.getTime()
        ? 'resize_session'
        : 'move_session') as 'move_session' | 'resize_session',
    }
    ...
    try {
      ...
      const originalBlock = basePatch.find((block: any) => String(block.task_id) === edit.taskId)

      if (basePlan?.plan_id && originalBlock) {
        const nextStart = new Date(edit.after.start || '')
        const nextEnd = new Date(edit.after.end || '')
        const mondayIndex = (nextStart.getDay() + 6) % 7
        const nextPatch = basePatch.map((block: any) => {
        ...
        await apiRequest('/api/plans/revisions/propose', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            base_plan_id: basePlan.plan_id,
            week_id: basePlan.week_id,
            plan_patch: nextPatch,
            source: 'calendar_drag',
            request_id: `calendar-revision-${crypto.randomUUID()}`,
            local_adjustment: {
              task_id: edit.taskId,
              block_id: originalBlock.block_id,
              event_type: edit.eventType,
              before: edit.before,
              after: edit.after,
            },
          }),
        })
        toast('Draft change is ready for review.')
        router.push('/app/plan')
      } else {
        event.revert()
        toast('This item is not part of the confirmed plan and cannot be moved here.')
      }
    } catch (error) {
      event.revert()
      const detail = error instanceof Error ? error.message : 'hard scheduling constraints'
      toast(`Couldn't move — ${detail}`)
    } finally {
