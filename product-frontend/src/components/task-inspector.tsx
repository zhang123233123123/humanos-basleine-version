...
function InspectorContent({ task, onSave, onOpenFocus }: TaskInspectorProps) {
...
export function TaskInspector({ task, onSave, onOpenFocus }: TaskInspectorProps) {
  ...
  return (
    <aside className="w-72 shrink-0 border-l border-border h-full flex flex-col bg-background overflow-y-auto" suppressHydrationWarning>
      {mounted ? <InspectorContent task={task} onSave={onSave} onOpenFocus={onOpenFocus} /> : null}
    </aside>
  )
}
