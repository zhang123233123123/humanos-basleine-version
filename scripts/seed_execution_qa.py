...
store.upsert_profile({
    ...
    "weekly_context": {
        "week_id": week_start,
        "weekly_available_windows": "每天 08:00-21:00",
        "available_windows": [{"day_index": day, "start": 8.0, "end": 21.0} for day in range(7)],
        "context_items": [],
        "keep_buffer": False,
    },
})
deadline = f"{(datetime.fromisoformat(week_start).date() + timedelta(days=6)).isoformat()} 23:00"
first = store.create_task(user_id, {"title": "Analyze interview transcripts", "due": deadline, "duration": 45, "priority": "high", "expected_difficulty": 6})
second = store.create_task(user_id, {"title": "Prepare findings diagram", "due": deadline, "duration": 45, "priority": "medium", "expected_difficulty": 4})
