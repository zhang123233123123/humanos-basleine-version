...
async function moveCalendarSession(taskId, blockId, dayIndex, startHour, requestedEnd = null) {
  ...
  if (!backendOnline || !confirmedSchedulePlan?.plan_patch?.length) {
    showProductToast("Could not create a calendar revision while the backend is unavailable.");
    render();
    return;
  }
  const revisedPatch = confirmedSchedulePlan.plan_patch.map((block) => (
    String(block.block_id) === String(blockId)
      ? { ...block, day_index: Number(dayIndex), start: target.start, end: target.end, session_minutes: target.session_minutes, draft_changed: true }
      : { ...block }
  ));
  try {
    const response = await api("/api/plans/revisions/propose", {
      method: "POST",
      body: JSON.stringify({
        user_id: currentUserId(),
        base_plan_id: confirmedSchedulePlan.plan_id,
        week_id: confirmedSchedulePlan.week_id,
        plan_patch: revisedPatch,
        source: requestedEnd === null ? "confirmed_calendar_drag" : "confirmed_calendar_resize",
        local_adjustment: {
          scope: "confirmed_calendar_edit",
          changed_block_ids: [String(blockId)],
          affected_task_ids: [String(taskId)]
        },
        request_id: crypto.randomUUID?.() || `calendar-revision-${Date.now()}`
      })
    });
    pendingSchedulePlan = response.plan;
    pendingSchedulePlan.base_plan_id = confirmedSchedulePlan.plan_id;
    await recordPlanEditEvent({
      eventType: requestedEnd === null ? "move_session" : "resize_session",
      taskId, blockId, before: snapshot, after: target,
      validation: { valid: true, advisories: calendarBlockAdvisories({ ...target, task, source: "scheduled" }, blockId) }
    });
    activeSelectionMode = "auto";
    rightRailMode = "plan";
    showProductToast("Change ready — review and apply it to the calendar.");
  } catch (error) {
    showProductToast(error.message || "The calendar change could not be validated.");
  }
  render();
}
...
rejectScheduleBtn.addEventListener("click", async () => {
  ...
  if (backendOnline && pendingSchedulePlan?.base_plan_id && pendingSchedulePlan?.plan_id) {
    try {
      const response = await api("/api/plans/revisions/cancel", {
        method: "POST",
        body: JSON.stringify({
          user_id: currentUserId(),
          plan_id: pendingSchedulePlan.plan_id,
          base_plan_id: pendingSchedulePlan.base_plan_id
        })
      });
      if (response.plan) confirmedSchedulePlan = response.plan;
    } catch (error) {
      showProductToast(error.message || "The draft revision could not be cancelled.");
      return;
    }
  }
  pendingSchedulePlan = null;
  showProductToast("Draft changes cancelled. The confirmed calendar was restored.");
  render();
});
...
async function resumeInterruptedTask(task) {
  ...
  try {
    ...
    if (backendOnline && session?.execution_session_id) {
      const impactResponse = await api("/api/schedules/local-adjustment", {
        method: "POST",
        body: JSON.stringify({
          user_id: currentUserId(), action: "resume", task_id: task.id, use_ai: true,
          execution_session_id: session.execution_session_id,
          remaining_minutes: remainingMinutes,
          request_id: crypto.randomUUID?.() || `resume-impact-${Date.now()}`
        })
      });
      const impact = impactResponse.evaluation || {};
      if (!impact.can_resume_directly && impact.requires_adjustment) {
        const adjust = window.confirm(`You have ${remainingMinutes} minutes remaining. Continuing now may affect ${impact.affected_task_ids?.length || 1} later session(s). Continue and review an adjustment draft?`);
        if (!adjust) {
          showProductToast("The current calendar was kept. Choose another resume time when ready.");
          rightRailMode = "plan";
          activeSelectionMode = "auto";
          render();
          return;
        }
        const revisionResponse = await api("/api/schedules/local-adjustment", {
          method: "POST",
          body: JSON.stringify({
            user_id: currentUserId(), action: "resume", choice: "continue_and_adjust", use_ai: true,
            task_id: task.id, execution_session_id: session.execution_session_id,
            remaining_minutes: remainingMinutes,
            request_id: crypto.randomUUID?.() || `resume-revision-${Date.now()}`
          })
        });
        if (revisionResponse.evaluation?.proposed_plan) {
          pendingSchedulePlan = revisionResponse.evaluation.proposed_plan;
          rightRailMode = "plan";
          activeSelectionMode = "auto";
          showProductToast("Resume adjustment ready — review the changed times first.");
          render();
          return;
        }
      }
...
async function requestPauseBackendPlan(task, resumeAt = null, mode = "resume") {
  if (!backendOnline) return { plan: null, error: "The backend is unavailable." };
  try {
    const response = await api("/api/schedules/local-adjustment", {
      method: "POST",
      body: JSON.stringify({
        user_id: currentUserId(), action: "pause", task_id: task.id, use_ai: true,
        execution_session_id: currentExecutionState?.session?.execution_session_id || null,
        remaining_minutes: taskWorkRemainingMinutes(task),
        desired_resume_at: resumeAt?.toISOString() || null,
        replan: mode === "reschedule",
        request_id: crypto.randomUUID?.() || `pause-adjustment-${Date.now()}`
      })
    });
    const plan = response.evaluation?.proposed_plan || null;
    return plan ? { ...plan, plan, error: null } : { plan: null, error: response.evaluation?.reason || null };
  } catch (error) {
    return { plan: null, error: error.message || "No valid adjustment could be generated." };
  }
}
...
document.getElementById("updateTodayAfterEarlyBtn")?.addEventListener("click", async () => {
  ...
  try {
    const response = await api("/api/schedules/local-adjustment", {
      method: "POST",
      body: JSON.stringify({
        user_id: currentUserId(), action: "early_finish", choice: "review_today", use_ai: true,
        request_id: crypto.randomUUID?.() || `early-finish-review-${Date.now()}`
      })
    });
    if (response.evaluation?.proposed_plan) {
      pendingSchedulePlan = response.evaluation.proposed_plan;
      recalculateAllPendingTaskSessions();
    } else {
      showProductToast(response.evaluation?.reason || "No later session needs to move.");
    }
    render();
  } catch (error) {
    showProductToast(error.message || "Today's plan could not be reviewed.");
  }
});
...
dailyCheckInForm?.addEventListener("submit", async (event) => {
  ...
  const runtimeState = await saveRuntimeStateToBackend({ daily_checkin: true, daily_note: changeNote }).catch((error) => { console.error(error); return null; });
  if (backendOnline && runtimeState) {
    try {
      const response = await api("/api/schedules/local-adjustment", {
        method: "POST",
        body: JSON.stringify({
          user_id: currentUserId(), action: "daily_checkin", runtime_state: runtimeState, use_ai: true,
          request_id: crypto.randomUUID?.() || `daily-checkin-evaluation-${Date.now()}`
        })
      });
      if (response.evaluation?.proposed_plan) pendingSchedulePlan = response.evaluation.proposed_plan;
      showProductToast(response.evaluation?.reason || "Today's next session was evaluated.");
    } catch (error) {
      console.error(error);
    }
  }
