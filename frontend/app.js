...
function setWizardStep(step) {
  ...
  const meta = {
    ...
    tasks: ["Add this week's work", "DeepSeek proposes sessions only inside the times you just provided."]
  ...
  if (structured.length) {
    return structured.map((item) => {
      ...
      return {
        ...
        shift_minutes: Number(item.shift_minutes || 30),
...
function recurringRoutineIntervals(profile = currentProfile) {
  ...
  parsedWeeklyActivities(profile).filter((item) => item.type === "recurring_routine").forEach((item) => {
    ...
    const shiftHours = Math.max(0, Number(item.shift_minutes || 30)) / 60;
    days.forEach((day) => {
      ...
      availability_window: { start: Math.max(0, range.start - shiftHours), end: Math.min(24, range.end + shiftHours) },
      movable: true,
      shift_minutes: Math.round(shiftHours * 60),
      inferred: item.source === "ai_default_suggestion" || !item.range,
      confidence: item.confidence || (defaults ? "low" : "high"),
      evidence: exception?.reason || (item.evidence?.length ? item.evidence.join("；") : `习惯时段默认保留，紧急情况下可前后微调 ${Math.round(shiftHours * 60)} 分钟`)
    });
    });
  });
  return { intervals, uncertain };
...
async function loadBackendState() {
  try {
    ...
    setEngineStatus(
      ...
      health.ai_enabled ? "DeepSeek proposes; Python validates hard constraints" : "Backend online; model API key is not configured"
    );
...
function calendarBlockViolations(block, ignoreBlockId = null) {
  ...
  if (!inside) violations.push("Outside this week's availability or inside protected buffer time");
  ...
  if (hardConflict) violations.push(`Conflicts with “${hardConflict.label}”`);
  const now = appNow();
  const todayIndex = (now.getDay() + 6) % 7;
  const nowHour = now.getHours() + now.getMinutes() / 60;
  if (block.day_index === todayIndex && block.start < nowHour) violations.push("Cannot move a session into the past");
  ...
  if (deadlineDay !== null && block.day_index > deadlineDay) violations.push("Falls after the task deadline");
  if (deadlineDay !== null && block.day_index === deadlineDay && deadlineHour !== null && block.end > deadlineHour + 0.001) violations.push("Ends after the task deadline");
  ...
  if (overlap) violations.push(`Overlaps “${overlap.task?.title || overlap.label}”`);
  const dependencies = [
  ...
  dependencies.forEach((dependency) => {
    ...
    if (Math.max(...before.map(ordinalEnd)) > Math.min(...after.map(ordinalStart)) + 0.001) {
      ...
      violations.push(`A hard dependency requires “${beforeTitle}” to finish before “${afterTitle}”`);
    }
  });
  return violations;
}
...
async function moveCalendarSession(taskId, blockId, dayIndex, startHour, requestedEnd = null) {
  ...
  if (pendingBlock) {
    ...
    const violations = calendarBlockViolations({ ...pendingBlock, task, source: "pending" }, blockId);
    if (violations.length) {
      Object.assign(pendingBlock, snapshot);
      await recordPlanEditEvent({ eventType: "edit_attempt_failed", taskId, blockId, before: snapshot, after: { ...pendingBlock }, validation: { valid: false, violations }, effective: false });
      setBackendStatus(`${task.title} was not changed: ${violations[0]}`, false);
    } else {
  ...
  const violations = calendarBlockViolations({ ...target, task, source: "scheduled" }, blockId);
  if (violations.length) {
    Object.assign(target, snapshot);
    addChatMessage("ai", "Constraint check failed", `${task.title} was not changed: ${violations.join("; ")}.`);
    render();
    return;
  }
...
async function requestTentativeSchedule(reason = "Generate a plan from the current inputs", targetTasks = null, planningOverrides = {}) {
  ...
  scheduleRequestInFlight = (async () => {
    try {
      ...
      setEngineStatus(
        pendingSchedulePlan.llm_provider === "deepseek" ? "DeepSeek" : "Rules",
        pendingSchedulePlan.llm_provider === "deepseek",
        pendingSchedulePlan.llm_provider === "deepseek" ? "DeepSeek proposes time blocks; Python validates hard constraints" : "This plan was generated by the deterministic constraint scheduler"
      );
...
function checkTaskTimePrompts(force = false) {
  ...
  tasks.forEach((task) => taskSlotSessions(task).filter((session) => session.day_index === todayIndex).forEach((session) => {
    ...
    if ((force || Math.abs(nowHour - session.end) <= 0.08) && !promptedSlots.has(endKey)) {
      ...
      if (task.status !== "completed") {
        addChatMessage("ai", "Session ended", `${task.title} is not marked complete. Add execution feedback, or pause and save your progress and next action.`);
      }
    }
  }));
  save();
...
function updateContextEventDialogMode() {
  ...
  document.getElementById("contextEventHint").textContent = flexible
      ? "A day and duration are enough. Add a possible range to keep DeepSeek strictly inside it."
    : routine
...
function renderActiveTask() {
  ...
  activeTask.innerHTML = `
    ...
    <div class="task-actions">
      ${isInterrupted ? `<button class="primary" type="button" data-action="resume-active-task">${task.status === "blocked" ? "Resume now" : "Resume task"}</button>` : task.status !== "completed" ? '<button class="ghost" type="button" data-action="pause-active-task">Pause</button>' : ''}
  ...
  activeTask.querySelector('[data-action="complete-active-task"]')?.addEventListener("click", () => {
    document.getElementById("feedbackActualMinutes").value = Math.min(task.duration || 45, 45);
...
function todayExecutionBlocks() {
  ...
  return visibleCalendarBlocks()
    .filter((block) => Number(block.day_index) === todayIndex && block.source !== "pending")
...
function renderExecutionRailDetails() {
  ...
  const after = blocks.filter((block) => String(block.block_id || "") !== currentBlockId && Number(block.end) > currentHour).slice(0, 2);
...
function renderNowCard() {
  ...
  const nextStep = valueList(task.checkpoints).slice().reverse().find((item) => /next/i.test(String(item.label || "")))?.text || task.context_window?.next_action;
  ...
  if (rightRailMode === "task" && ["now", "paused"].includes(effectiveMode)) {
    ...
    nowCard.innerHTML = `<span class="review-eyebrow">Session ended</span><h2>${escapeHtml(task.title)}</h2><p>Planned: ${plannedStart}–${plannedEnd}</p><strong>How did it go?</strong><div class="feedback-shortcuts"><button type="button" data-feedback-outcome="completed">Completed this task</button><button type="button" data-feedback-outcome="partial">Made some progress</button><button type="button" data-feedback-outcome="not_started">Did not start</button></div><div class="button-row"><button class="ghost" type="button" data-execution-action="start">Continue a little longer</button><button class="ghost" type="button" data-execution-action="pause">Pause and save context</button></div>`;
  } else {
  ...
  nowCard.querySelectorAll("[data-feedback-outcome]").forEach((button) => button.addEventListener("click", () => {
    selectTask(task.id, "manual");
    document.getElementById("feedbackCompletion").value = button.dataset.feedbackOutcome;
    document.getElementById("feedbackActualMinutes").value = button.dataset.feedbackOutcome === "not_started" ? 0 : elapsed;
...
async function pauseCurrentExecution() {
  ...
  executionActionInFlight = true;
  const actualMinutes = executionElapsedMinutes(session);
  try {
    if (backendOnline) {
      const response = await api("/api/execution-sessions/pause", { method: "POST", body: JSON.stringify({ user_id: currentUserId(), execution_session_id: session.execution_session_id, actual_minutes: actualMinutes, request_id: `pause-${session.execution_session_id}-${Date.now()}` }) });
...
async function finishCurrentExecution() {
  ...
  try {
    if (backendOnline) {
      const response = await api("/api/execution-sessions/end", { method: "POST", body: JSON.stringify({ user_id: currentUserId(), execution_session_id: session.execution_session_id, actual_minutes: actualMinutes, request_id: `finish-${session.execution_session_id}-${Date.now()}` }) });
      currentExecutionState = { ...currentExecutionState, mode: "session_ended", session: response.execution_session };
    }
    selectTask(task.id, "manual");
    document.getElementById("feedbackActualMinutes").value = actualMinutes;
    feedbackDialog.dataset.executionSessionId = session.execution_session_id;
    if (!feedbackDialog.open) feedbackDialog.showModal();
    render();
  } finally { executionActionInFlight = false; }
...
async function acceptParallelSuggestion(suggestionId) {
  ...
  try {
    ...
    if (new Set(groupBlocks.map((block) => String(block.task_id))).size !== 2) {
      throw new Error("DeepSeek did not return a valid two-task parallel group after constraint checking.");
    }
    setBackendStatus("Plan updated", backendOnline);
    pendingReviewKey = currentPendingReviewKey();
    render();
...
function renderPendingSchedule() {
  ...
  const mandatoryDecisions = [
    ...
    ...validationViolations.map((item) => ({
      ...
      task_id: item.task_id,
      message: analysisDisplayText(item.detail || item.constraint || item.message || "A hard scheduling constraint still conflicts with the draft.")
    })),
    ...ambiguityDecisions
  ];
  ...
  if (planReviewCount) {
    const conflictCount = unscheduledDecisions.length + validationViolations.length;
    const ambiguityLabel = ambiguityDecisions.length ? ` · ${ambiguityDecisions.length} decision${ambiguityDecisions.length === 1 ? "" : "s"}` : "";
    ...
    planReviewCount.textContent = `${reviewTaskIds.length} tasks · ${conflictCount} conflict${conflictCount === 1 ? "" : "s"}${ambiguityLabel}${suggestionLabel}`;
    planReviewCount.classList.toggle("complete", hasDraftTasks && mandatoryDecisions.length === 0);
  }
  ...
  confirmScheduleBtn.textContent = "Add plan to calendar";
  ...
  const selectedCandidate = candidates.find((candidate) => candidate.id === pendingSchedulePlan.selected_candidate_id) || candidates[0] || null;
  const selectedMetrics = selectedCandidate?.metrics || {};
  const mandatoryDecisionCards = mandatoryDecisions.map((decision) => `
    <article class="plan-decision-card">
      <div><i aria-hidden="true"></i><strong>Needs your decision</strong></div>
      <p>${escapeHtml(decision.message)}</p>
      <div class="decision-actions">
        <button class="ghost" type="button" data-plan-decision="availability">Add available time</button>
        ${decision.task_id ? `<button class="ghost" type="button" data-plan-decision="deadline" data-task-id="${escapeHtml(decision.task_id)}">Move deadline</button><button class="ghost" type="button" data-plan-decision="scope" data-task-id="${escapeHtml(decision.task_id)}">Reduce scope</button>` : `<button class="ghost" type="button" data-plan-decision="ask">Ask HumanOS</button>`}
      </div>
    </article>`).join("");
  pendingScheduleText.innerHTML = `
    ${pendingSchedulePlan.confirmation_error ? `<div class="plan-warning confirmation-error" role="alert"><strong>${escapeHtml(pendingSchedulePlan.confirmation_error.title || "Calendar could not be created")}</strong><span>${escapeHtml(pendingSchedulePlan.confirmation_error.detail || "Review the highlighted issue and try again.")}</span></div>` : ""}
    ${selectedCandidate ? `<section class="recommended-plan-card">
      <span>Recommended plan</span>
      <strong>${escapeHtml(localizeDisplayTime(selectedCandidate.label || "Best-fit plan", true))}</strong>
      <p>${escapeHtml(localizeDisplayTime(pendingSchedulePlan.user_reason || pendingSchedulePlan.explanation || (Number(selectedMetrics.remaining_minutes || 0) > 0 ? `${Number(selectedMetrics.remaining_minutes)} minutes need your decision.` : "The plan protects demanding work for your stronger focus periods and keeps lighter work for lower-energy periods."), true))}</p>
    </section>` : ""}
...
function handlePlanDecisionAction(button) {
  ...
  if ((action === "deadline" || action === "scope") && task) {
    selectTask(task.id, "manual");
    openTaskDialog(task);
    return;
  }
  if (action === "ask" && chatDrawer) {
...
confirmScheduleBtn.addEventListener("click", async () => {
  ...
  const hardViolations = pendingSchedulePlan.plan_patch.flatMap((block) => {
    ...
    return target ? calendarBlockViolations({ ...block, task: target, source: "pending" }, block.block_id).map((message) => ({ block_id: block.block_id, message })) : [];
  });
  if (hardViolations.length) {
    addChatMessage("ai", "Plan failed constraint validation", hardViolations.map((item) => item.message).join("; "));
    ...
    pendingSchedulePlan.confirmation_error = {
      ...
      detail: hardViolations.map((item) => item.message).join("; ")
    };
    render();
    return;
  }
  ...
  if (backendOnline) {
    ...
    if (!backendValidation.valid) {
      ...
      const detail = backendValidation.violations.map((item) => item.detail || item.constraint || item.type).join("; ");
      pendingSchedulePlan.confirmation_error = { title: "Python constraint validation failed", detail };
      addChatMessage("ai", "Python constraint validation failed", detail);
      render();
      return;
    }
...
function preparePauseDialog(task) {
  ...
  document.getElementById("pauseProgress").value = ["No progress has been recorded yet.", "尚未记录当前进展。"].includes(context.progress) ? "" : context.progress;
  document.getElementById("pauseNextStep").value = ["Confirm the task goal, then choose one step that fits within 15–30 minutes.", "回来后先确认任务目标，再选择一个 15-30 分钟内可完成的小步骤。"].includes(context.nextStep) ? "" : context.nextStep;
  document.getElementById("pauseActualMinutes").value = "0";
...
async function resumeInterruptedTask(task) {
  ...
  try {
    ...
    task.status = remainingMinutes <= 0 ? "completed" : "queued";
    task.contextWindow = {
    ...
    await patchBackendTask(task);
    addChatMessage("ai", "Resume task", `${reentry.prompt}\nStart with a ${reentry.suggested_block_minutes || 45}-minute re-entry session.`);
    if (task.status !== "completed" && missingTimeConfirmationFields(task).length === 0) {
      ...
      calendarView = "week";
      await requestTentativeSchedule("Resume the interrupted task and replan using every task's exact remaining work duration.", replannable);
    }
  } catch (error) {
    console.error(error);
...
pauseForm.addEventListener("submit", async (event) => {
  ...
  const actualMinutes = Math.max(0, Math.min(Number(document.getElementById("pauseActualMinutes").value) || 0, previousRemaining));
  const remainingMinutes = Math.max(previousRemaining - actualMinutes, 0);
  const originalMinutes = Number(task.execution?.original_estimate_minutes ?? task.duration ?? previousRemaining);
  const accumulatedActual = Number(task.execution?.accumulated_actual_minutes || 0) + actualMinutes;
  const progressPercent = originalMinutes > 0 ? Math.min(100, Math.round(((originalMinutes - remainingMinutes) / originalMinutes) * 100)) : 0;
  ...
  if (backendOnline) {
    await api("/api/context-dumps", {
      ...
      body: JSON.stringify({
        ...
        progress, next_action: nextAction, remaining_duration_minutes: remainingMinutes,
  ...
  if (calendarAction === "replan") {
    ...
    if (replannable.length) {
      calendarView = "week";
      await requestTentativeSchedule("After the interruption, compare the remaining tasks using the released time.", replannable);
    }
  }
  activeId = task.id;
...
feedbackForm.addEventListener("submit", async (event) => {
  ...
  const payload = {
    ...
    task_evaluation: {
      completion,
      actual_minutes: Number(document.getElementById("feedbackActualMinutes").value) || 0,
      perceived_difficulty: Number(document.getElementById("feedbackDifficulty").value) || null,
      remaining_duration_minutes: completion === "completed" ? 0 : Math.max(taskWorkRemainingMinutes(task) - Number(document.getElementById("feedbackActualMinutes").value || 0), 0)
    },
    state_evaluation: {
  ...
  if (backendOnline) await api("/api/execution-feedback", { method: "POST", body: JSON.stringify(payload) });
  ...
  task.execution = {
    ...
    accumulated_actual_minutes: Number(task.execution?.accumulated_actual_minutes || 0) + payload.task_evaluation.actual_minutes,
    remaining_duration_minutes: payload.task_evaluation.remaining_duration_minutes,
