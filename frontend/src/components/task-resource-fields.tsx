...
export function TaskResourceFields({ locale, resourceTags, attentionMode, onResourceTagsChange, onAttentionModeChange, sourceText }: {
  locale: string
  ...
  return <div className="grid gap-4 rounded-xl border p-4">
    <div>
      <p className="text-sm font-medium">{zh ? '任务占用的资源' : 'Task resource tags'}{sourceText && <span className="ml-1 text-[9px] font-normal text-muted-foreground">{sourceText}</span>}</p>
    ...
    <label className="grid gap-1.5 text-sm">
      <span className="font-medium">{zh ? '注意参与方式' : 'Attention mode'}{sourceText && <span className="ml-1 text-[9px] font-normal text-muted-foreground">{sourceText}</span>}</span>
