...
import type { LearningResourceEnvelope, ProfileTraitEvidenceItem, ProfileTraitEvidenceTrace } from '@/lib/contracts/insights-contracts'
...
function EvidenceRecord({ item, locale }: { item: ProfileTraitEvidenceItem; locale: string }) {
  const zh = locale === 'zh'
...
export function ProfileTraitEvidencePanel({ traitId, locale }: { traitId: string; locale: string }) {
  const zh = locale === 'zh'
  ...
  return <section className="mt-4 border-t border-stone-200 pt-4 dark:border-white/10">
    ...
    {trace.evidence.length === 0 ? <p className="mt-4 rounded-2xl border border-dashed p-5 text-center text-sm text-stone-500">{zh ? '目前没有可展示的证据。' : 'No evidence is currently available.'}</p> : <div className="mt-4 grid gap-3">{trace.evidence.map((item) => <EvidenceRecord key={item.evidence_id} item={item} locale={locale} />)}</div>}
  </section>
}
