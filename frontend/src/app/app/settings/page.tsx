      ...
      <div className="mx-auto max-w-6xl px-4 py-8 md:px-8 md:py-12">
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          ...
          <section className="min-h-[520px] rounded-3xl border border-stone-300/70 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#101720] md:p-8">
            {section === 'account' && <SettingsSection title={c('个人资料', 'Profile')} description={c('这些信息用于确定身份语境和所有时间计算。', 'These fields establish identity context and time calculations.')}><Field label={c('登录邮箱', 'Email')}><Input value={session?.user?.email || profile.user_id || ''} disabled /></Field><Field label={c('主要角色', 'Primary role')}><Input value={profile.role || ''} onChange={(event) => update('role', event.target.value)} placeholder={c('例如：研究生、产品经理', 'e.g. Research student, product manager')} /></Field><Field label={c('时区', 'Timezone')}><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={profile.timezone || 'Asia/Shanghai'} onChange={(event) => update('timezone', event.target.value)}><option value="Asia/Shanghai">Asia/Shanghai</option><option value="Asia/Hong_Kong">Asia/Hong_Kong</option><option value="Asia/Singapore">Asia/Singapore</option><option value="Europe/London">Europe/London</option><option value="America/New_York">America/New_York</option><option value="America/Los_Angeles">America/Los_Angeles</option></select></Field><div className="rounded-2xl border p-4"><p className="text-sm font-medium">{c('已确认画像', 'Confirmed traits')}</p><p className="mt-1 text-xs leading-5 text-stone-500">{c('查看、编辑、暂停或遗忘系统已获你确认的长期画像。', 'View, edit, pause, or forget long-term traits you have confirmed.')}</p><Button asChild variant="outline" className="mt-3"><Link href="/app/settings/profile">{c('管理画像', 'Manage traits')}<ChevronRight className="ml-2 h-4 w-4" /></Link></Button></div></SettingsSection>}
            {section === 'rhythm' && <SettingsSection title={c('工作节律', 'Working Rhythm')} description={c('系统会优先把高负荷任务放在高效窗口，并保护恢复时间。', 'HumanOS prioritizes demanding work in effective windows and protects recovery time.')}><div className="grid gap-5 md:grid-cols-2"><Field label={c('深度工作窗口', 'Deep work window')} hint="09:00-11:30"><TimeRangeInput value={profile.deep_work_window || '09:00-11:30'} onChange={(value) => update('deep_work_window', value)} /></Field><Field label={c('低能量窗口', 'Low-energy window')} hint="14:00-15:30"><TimeRangeInput value={profile.low_energy_window || '14:00-15:30'} onChange={(value) => update('low_energy_window', value)} /></Field></div><Notice>{c('节律变化只用于新提案；已确认日历不会被静默修改。', 'Rhythm changes apply to new proposals only; confirmed calendars are never changed silently.')}</Notice></SettingsSection>}
...
function TimeRangeInput({ value, onChange }: { value: string; onChange: (value: string) => void }) { const [start = '09:00', end = '17:00'] = String(value || '').split('-'); return <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2"><input aria-label="Start time" type="time" className="h-10 rounded-md border bg-background px-3 text-sm" value={start} onChange={(event) => onChange(`${event.target.value}-${end}`)} /><span className="text-stone-400">-</span><input aria-label="End time" type="time" className="h-10 rounded-md border bg-background px-3 text-sm" value={end} onChange={(event) => onChange(`${start}-${event.target.value}`)} /></div> }
