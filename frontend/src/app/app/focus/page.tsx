...
export default function FocusPage() {
  ...
  const takeBreak = async (minutes: number) => {
    ...
    try {
      ...
      const result = await apiRequest<ExecutionResourceEnvelope<{ execution_session: ExecutionSession; pause_review: ExecutionImpact }>>('/api/execution-sessions/pause', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ execution_session_id: session.execution_session_id, actual_minutes: activeMinutes, remaining_minutes: displayRemaining, pause_reason: 'normal_break', resume_preference: 'soon', preferred_resume_at: resumeAt, break_minutes: minutes, request_id: requestId('break') }),
      })
      setCurrent({ mode: 'paused', session: result.data.execution_session, task })
      ...
      <div className="humanos-operating-container mx-auto max-w-6xl space-y-6">
        ...
        {endedSession ? (
          <Card className="border-primary/30">
            <CardHeader><CardTitle>Session ended · {title}</CardTitle><CardDescription>{actualMinutes} min tracked automatically. How did it go?</CardDescription></CardHeader>
            <CardContent className="grid gap-5 md:grid-cols-2">
              <label className="grid gap-2 text-sm md:col-span-2"><span>{t('execution.completion')}</span><select className="h-10 rounded-md border bg-background px-3" value={completion} onChange={(event) => setCompletion(event.target.value as typeof completion)}>{actualMinutes > 0 ? <><option value="completed">Finished the whole task</option><option value="some_progress">Made progress</option><option value="no_progress">Worked, no progress</option></> : <><option value="did_not_start">Did not start</option><option value="some_progress">Start now</option></>}</select></label>
