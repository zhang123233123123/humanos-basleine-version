...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await humanosRequest('POST', '/api/execution-sessions/pause', { ...body, user_id: userId }) as { execution_session: unknown; interruption: unknown; pause_review: unknown }
    return Response.json({ data: { execution_session: result.execution_session, interruption: result.interruption, pause_review: result.pause_review }, resources: { self: '/api/execution-sessions/pause', tasks: '/api/tasks', current: '/api/execution-sessions/current' }, meta: { resource: 'execution_session', aggregate_root: 'task', read_only: false } })
  } catch (error) {
